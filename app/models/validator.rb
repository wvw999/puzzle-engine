...
class Validator
  ...
  def wipeword(word,board,solution)
    ...
    word.length.times do |update|
      ...
      puts "trying to remove #{letters[counter]} from board at #{solution[counter]}"
      puts "current value at that location is #{selected}"
  ...
  def solver_engine(words,board,solution)
    tempboard = []
    board.each do |makehash|
      tempboard.push Hash[:x, makehash[0].to_i, :y, makehash[1].to_i, :letter, makehash[2]]
    end
    ...
    completed = tempboard.select { |val| val[:letter] == " "}
    if completed.length == tempboard.length
      return 1
    else
      return 0
    end
